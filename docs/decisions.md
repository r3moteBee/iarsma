...
**Why:** Reverses the implementation-plan / Phase 0 brief assumption that Stalwart treats the iarsma client as confidential. The 2026-04-29 OIDC discovery doc against the reference deployment plus a manual check confirmed Stalwart accepts public clients (PKCE-only is the OAuth 2.1 BCP for SPAs anyway). Staying public removes a process from `just dev-all`, removes the secret-handling surface from the Phase 0 threat model, and keeps the codepath the same on web and Tauri.
